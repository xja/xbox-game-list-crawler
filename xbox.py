...
class XboxSpider(scrapy.Spider):
    ...
    def parse(self, response):
        for game in response.css('.m-channel-placement-item'):
            ...
            if review_count != None:# and review_count != '':
    ...
    def parseContent(self, response):
        ...
        if price_original == None:  # when an element does not exist, css return None
        ...
        if publisher != None or publisher != '': # NULL value:
            publisher = publisher.replace(' ','').replace('\r\n','')
        if developer != None or developer != '': # NULL value:
